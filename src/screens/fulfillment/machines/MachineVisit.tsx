...
import { FULFill_MACHINES } from "@/constants/Constant";
...
export default function FulfillmentMachineVisit() {
  ...
  useEffect(() => {
    // Fetch transactions when page, itemsPerPage, or selectedMonth changes
    fetchTransactions();
  }, [currentPage, itemsPerPage, selectedMonth]);

  const fetchMachineDetails = async () => {
    try {
      const res = await postRequest(`/fulfillment/machineDetailsWithMachineCode`, {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const res = await postRequest<TransactionResponse>(
        "/fulfillment/machineTransactionWithMachineCode",
        requestBody,
      );
  ...
  const fetchChartData = async (type: "monthly" | "weekly") => {
    ...
    try {
      const res = await postRequest<ApiResponse>(
        "/fulfillment/BarChartMainDashboardSanitaryByMachineCode",
        { machine_code: machine.machine_code },
      );
      ...
      setTotalChartTransactions(
        transformed.reduce((sum, d) => sum + d.transactions, 0),
      );
      setError(null);
    } catch (err) {
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const csvData = await postRequest(
        "/fulfillment/machine-transaction-csv",
        requestBody,
      );

      // The API returns CSV string directly
      ...
      // Include month in filename if selected
      const monthStr = selectedMonth ? `_${selectedMonth}` : "_all";
      const fileName = `transactions_${machine?.machine_code || "machine"}${monthStr}_${new Date().toISOString().split("T")[0]}.csv`;
  ...
  return (
    <div>
      <SiteHeader title="🌍 Fulfillment Machine Dashboard" />
      <div className="min-h-screen bg-gradient-to-b from-green-50 to-teal-50 p-6">
        <div className="mx-auto max-w-7xl">
          <Button
            ...
            onClick={() => navigate(FULFill_MACHINES)}
          >
          ...
          >
            <TabsList className="grid w-full grid-cols-5 mb-6 bg-green-100 rounded-xl p-1 shadow-sm">
              ...
              <TabsTrigger value="user-transactions">
                👥 Transactions
              </TabsTrigger>
                  ...
                  <CardContent>
                    <Card>
                          ...
                          <div>
                            ...
                            <div className="mt-3 text-sm text-muted-foreground space-y-1">
                              ...
                              <div>
                                ...
                                {totalChartTransactions.toLocaleString()}
                              </div>
                            </div>
                            <div className="flex p-2 space-x-2">
                              ...
                              <Button
                                variant={
                                  metric === "revenue" ? "default" : "outline"
                                }
                      ...
                      <CardContent className="h-[400px]">
                        {loading ? (
                          <div className="flex justify-center items-center h-full animate-pulse text-emerald-600">
                            ⏳ Loading...
                          </div>
                        ) : error ? (
                ...
                {userTransactions.length > 0 && (
                  <div className="flex items-center justify-between px-4 mt-4">
                    <div className="text-muted-foreground hidden flex-1 text-sm lg:flex">
                      Showing {userTransactions.length} of{" "}
                ...
                >
                  ...
                  <Card className="rounded-2xl shadow-lg">
                    <CardHeader>
                      ...
                      <CardDescription>
                        How users interact with this machine over time
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      ...
                      <Card className="border border-emerald-100">
                        <CardHeader className="pb-2">
                          ...
                          <CardDescription>
                            How often users return to this machine
                          </CardDescription>
                        </CardHeader>
                      ...
                      <Card className="border border-emerald-100">
                        <CardHeader className="pb-2">
                          ...
                          <CardDescription>
                            Most frequent users of this machine
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          <Table>
                  ...
                  <Card className="rounded-2xl shadow-lg">
                    <CardHeader>
                      ...
                      <CardDescription>
                        When and how often this machine is used
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      {/* Usage KPI Cards */}
