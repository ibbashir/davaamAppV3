...
const FulfillmentCashCollectionPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const result = await getRequest<{
        success?: boolean;
        data?: CashCollection[];
      }>(`/fulfillment/getAllCashCollection`);
  ...
  const fetchMachines = async () => {
    try {
      const result = await getRequest<{ data?: MachineInfo[] }>(
        "/fulfillment/getMachinesWithMachineCode",
      );
...
export default FulfillmentCashCollectionPage;
