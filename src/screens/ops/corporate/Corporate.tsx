...
const Corporate = () => {
  ...
  // Client states
  const [clientsData, setClientsData] = useState<Record<string, CorporateClient[]>>({
    ibex: [],
    tapal: [],
    jaffer: [],
    jpCoats: [],
  ...
  const getClientData = (tabName: string): CorporateClient[] => clientsData[tabName] || []
  ...
  const getFilteredClientData = (data: CorporateClient[]) => {
    return data.filter((item) => {
      ...
      const matchesDate = !dateFilter || moment(item.created_at).isSame(dateFilter, "day")
      const matchesMinBalance = !minBalance || item.balance >= Number(minBalance)
      const matchesMaxBalance = !maxBalance || item.balance <= Number(maxBalance)

      return matchesSearch && matchesDate && matchesMinBalance && matchesMaxBalance
    })
  }
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value)
    resetFilters()
    if (["ibex", "tapal", "getz", "jpCoats", "ebm", "mobilink", "jaffer"].includes(value)) {
  ...
  const uniqueClients = new Set(topupHistory.map((item) => item.name || item.corporate_name)).size
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">
                {topupHistory.filter((item) => moment(item.created_at).isSame(moment(), "month")).length}
              </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      A list of all the corporate topup history in your account including their name, amount, purpose
                      and date.
                    </CardDescription>
                  </div>
                      ...
                      {topupHistory.length > 0 ? (
                        topupHistory.map((entry) => (
                          <TableRow key={entry.id} className="hover:bg-muted/50">
                            <TableCell className="font-medium">{entry.corporate_name || entry.name}</TableCell>
